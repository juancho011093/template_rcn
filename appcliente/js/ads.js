   ...
   $.ajax({ url: "https://ads.canalrcn.tech/NewGetAllUnits.php?search=CANALRCN_Home_", success: function(ads){
       ...
       if(ads.length > 0){
           ...
           $.each(ads, function( indexads, value ) {
               ...
               sizes = "["+value.sizes[0].width+","+value.sizes[0].height+"]";
                   ...
                   scriptPUSH.innerHTML= scriptPUSH.innerHTML+ "googletag.defineSlot('/205320464/CANALRCN/HOME/"+value.name+"',"+sizes+", '"+value.name+"').addService(googletag.pubads());";
           });
